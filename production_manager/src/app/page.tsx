...
export default function Home() {
  ...
  const nofification_system =
    process.env.NOTIFICATION_SYSTEM || "localhost:55022";
  ...
  useEffect(() => {
    api.v1.robot.getAll().then((rs) => {
      ...
      sse.onerror = (e) => {
        setNotification(
          `Could not connect to notification system, retrying ... Attempt [${count++}/3]`
        );
        if (count > 4) {
          sse.close();
